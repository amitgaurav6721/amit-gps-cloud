...
def admin_panel():
    ...
    # --- TAB 1: REPORTS (FIXED DATABASE FETCH) ---
    with t1:
    ...
    with t2:
        ...
        if 'injecting' not in st.session_state: st.session_state.injecting = False
        ...
        if not st.session_state.injecting:
            ...
            while st.session_state.injecting:
                ...
                # --- DB LOGGING (Before sending to avoid lag) ---
                try:
                    # 'user_id' ko hata diya taaki Foreign Key error na aaye
                    supabase.table("activity_logs").insert({
                        "vehicle_no": f"SIM RUNNING: {v_no}",
                        "created_at": datetime.now().isoformat()
                    }).execute()
                except Exception as db_e:
                    st.sidebar.error(f"DB Write Fail: {db_e}")
