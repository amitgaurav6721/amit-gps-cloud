...
with st.sidebar:
    ...
    st.markdown("---")
    mode = st.radio("Select Operational Mode:", ["Static (Manual)", "Experimental (Live Auto)"])

# --- 4. COMMON UI (Location & Map) ---
...
# --- MODE A: STATIC MANUAL ---
if mode == "Static (Manual)":
    st.subheader("📝 Static Lab (Fixed Checksum Mode)")
    st.info("In strings mein date/time aur checksum fix hai. Sirf connectivity test ke liye use karein.")
    ...
    manual_packet = st.text_area("Final Raw Packet (Edit if needed):", value=static_templates[sel_template], height=100)
    ...
    if st.button("🚀 SEND STATIC PACKET"):
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                st.success(f"Static Packet Sent to {srv_ip}")
        except Exception as e:
            st.error(f"Static Send Failed: {e}")

# --- MODE B: EXPERIMENTAL AUTO ---
else:
    st.subheader("🧪 Turbo Discovery & Filter Mode")
    st.write("Jin tags par error aaye, unhe uncheck kar dein:")
    
    # Tag Selection Grid with Status
    selected_tags = []
    ...
    if col_btn1.button("🚀 START AUTO-ROTATOR", use_container_width=True):
        if not selected_tags:
            st.error("Bhai, kam se kam ek Tag select karo!")
        else:
    ...
    if st.session_state.running:
        ...
        while st.session_state.running:
            ...
            try:
                # Har packet ke liye fresh connection taaki status sahi pata chale
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.settimeout(3)
                    s.connect((srv_ip, srv_port))
                    ...
                    log_placeholder.success(f"Sent: {current_tag} | Latency: {latency}ms | Time: {t_live}")
            
            except Exception as e:
                st.session_state.tag_status[current_tag] = "❌"
                log_placeholder.error(f"Failed: {current_tag} | Error: {e}")
                # Connection reset hone par thoda wait karenge
                time.sleep(1)
            
            idx += 1
            time.sleep(interval)
            st.rerun() # UI refresh taaki ✅/❌ dikhe
