...
def transmission_engine(sel_tags, l_lat, l_lon):
    if st.session_state.running and sel_tags:
        ...
        try:
            ...
            st.session_state.logs.insert(0, f"{now.strftime('%H:%M:%S')} | 🔴 {e}")
...
with t1:
    ...
    with col_r:
        gap = st.slider("Interval (Sec)", 0.1, 5.0, 1.0)
...
with t2:
    ...
    b_mode = st.radio("Method", ["Auto", "Manual"], horizontal=True)
    ...
    if b_mode == "Auto":
        ...
        body = (
            f"PVT,{m_tag},2.1.1,NR,01,L,{imei_val},{veh_no},1,"
            f"{datetime.now().strftime('%d%m%Y')},{datetime.now().strftime('%H%M%S')},"
            f"{m_lat:.6f},N,{m_lon:.6f},E,"
            "0.00,0.0,0,0,0,0,airtel,1,1,12.0,4.0,0,C,0,0,0,0"
        )
        ...
        raw_in = st.text_area(
            "Body:",
            value=f"PVT,GRL,2.1.1,NR,01,L,{imei_val},{veh_no},1,{datetime.now().strftime('%d%m%Y')},{datetime.now().strftime('%H%M%S')},25.650945,N,84.784773,E,0.00,0.0,0,0,0,0,airtel,1,1,12.0,4.0,0,C,0,0,0,0"
        )

        clean = raw_in.strip().replace("$", "").split("*")[0]
        final_pkt = f"${clean},{get_checksum(clean)}*\r\n"
