...
# Direct Image Link (Size optimized for 200px)
...
def get_contact_details():
    try:
        ...
        return res.data[0] if res.data else {"whatsapp_no": "+91 XXXXX XXXXX", "email_id": "admin@example.com", "support_time": "10 AM - 8 PM", "upi_id": "amit@upi"}
    except:
...
def recharge_page():
    ...
    with col_form:
        ...
        c1, c2 = st.columns(2)
        confirm_cid = c1.text_input("Confirm CID", value=f"CID-{1000+cid}")
        mobile_no = c2.text_input("Mobile No", placeholder="10 digit number")
        ...
        if st.button("Submit Request", use_container_width=True):
            if utr and mobile_no:
                ...
                if len(existing.data) > 0:
                    st.error(f"❌ UTR `{utr}` pehle hi submit ho chuka hai.")
                else:
                    try:
                        supabase.table("recharge_requests").insert({
                            "username": st.session_state.user, "utr_number": utr, "amount": amt,
                            "mobile_no": mobile_no, "cid_display": confirm_cid
                        }).execute()
                        st.success("✅ Request Sent!"); time.sleep(2); st.session_state.page = "dashboard"; st.rerun()
                    except Exception as e:
                        st.error(f"System Error: {str(e)}")
            else: st.warning("Mobile aur UTR bharein.")
    if st.button("⬅️ Back"): st.session_state.page = "dashboard"; st.rerun()
    ...
    with menu[1]:
        ...
        if not st.session_state.admin_running:
            ...
            while st.session_state.admin_running:
                ...
                str_area.code(f"Preview: $PVT,{tags[0]},2.1.1,NR,01,L,{imei},{vno},1,{dt}...")
                for t in tags:
