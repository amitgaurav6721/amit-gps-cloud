...
def contact_us_page(reason="general"):
    ...
    if reason == "deactivated":
        st.error("⚠️ Aapka account Deactivated hai. Dubara active karne ke liye Recharge karein ya Support se sampark karein.")
...
def recharge_page():
    ...
    with col_form:
        ...
        amt = st.selectbox("Choose Plan", plan_list if plan_list else ["Standard Plan"])
        ...
        if is_pending:
            ...
            if st.button("Submit Recharge Request", use_container_width=True):
                if utr and len(mobile_no) == 10:
                    ...
                    st.warning("Kripya sahi 10-digit Mobile No aur UTR bharein.")
...
def admin_panel():
    ...
    with menu[0]:
            ...
            if u_search.data:
                for usr in u_search.data:
                    ...
                    with st.expander(f"👤 {usr['username']} (CID-{1000 + usr['cid_id']})"):
                        st.write(f"Days Left: {d_left} | Status: {status.upper()} | Exp: {usr['expiry_date']}")
                        c1, c2, c3 = st.columns(3)
                        if c1.button("+28 Days", key=f"e1_{usr['username']}"):
                            curr_exp = datetime.strptime(usr['expiry_date'], '%Y-%m-%d')
                        ...
                        if c2.button("+84 Days", key=f"e3_{usr['username']}"):
                            curr_exp = datetime.strptime(usr['expiry_date'], '%Y-%m-%d')
                            new_exp = (max(curr_exp, datetime.now()) + timedelta(days=84)).strftime("%Y-%m-%d")
                            supabase.table("user_profiles").update({"expiry_date": new_exp}).eq("username", usr['username']).execute(); st.rerun()
        ...
        with st.form("new_user_form"):
            ...
            selected_plan = st.selectbox("Assign Initial Plan", plan_options if plan_options else ["Standard (28 Days)"])
            lat, lon = st.number_input("Latitude", 25.5941, format="%.7f"), st.number_input("Longitude", 85.1376, format="%.7f")
    ...
    with menu[1]:
        st.subheader("🚀 Master Injector (Live Stream)")
        vno, imei = st.text_input("Vehicle No (Master)").upper(), st.text_input("IMEI (Master)")
        ...
        if not st.session_state.admin_running:
            ...
            while st.session_state.admin_running:
                ...
                for i, t in enumerate(tags):
                    full_p = f"$PVT,{t},2.1.1,NR,01,L,{imei},{vno},1,{dt},{lat_m:.7f},N,{lon_m:.7f},E,0.00,0.0,11,73,0.8,0.8,airtel,1,1,11.5,4.3,0,C,26,404,73,0a83,e3c8,e3c7,0a83,7,e3fb,0a83,7,c79d,0a83,10,e3f9,0a83,0,0001,00,000041,DDE3*"
                    str_area.text_area(f"Injecting [{i+1}/{len(tags)}]: {t}", value=full_p, height=100)
                    th = threading.Thread(target=send_packet_thread, args=("vlts.bihar.gov.in", 9999, full_p, res, True))
                    th.start(); th.join()
        ...
        if reqs.data:
            ...
            for r in reqs.data:
                if st.button(f"Approve {r['username']} (UTR: {r['utr_number']})", key=f"app_{r['id']}"):
                    try:
                        days = int(r['amount'].split('(')[1].split(' ')[0])
                    except: days = 28
    ...
    with menu[3]:
        ...
        with st.form("settings_form"):
            ...
            if st.form_submit_button("Save All Settings"):
                ...
                st.success("Saved Live!"); st.rerun()

    with menu[4]:
        st.subheader("Manage Tags")
        ta = st.text_input("Add New Tag Name")
        if st.button("➕ Add Tag"):
            if ta: add_new_tag(ta); st.success(f"Added {ta}"); st.rerun()
        st.divider()
        tags = get_tags()
        for t in tags:
