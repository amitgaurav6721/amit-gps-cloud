...
# --- 4. LOGIN LOGIC (Fixed for Dual Login) ---
        ...
        if st.form_submit_button("Login", use_container_width=True):
            try:
                ...
                if res.user:
                    ...
                    st.rerun() # Refresh app to enter dashboard
                else: st.error("Invalid Credentials")
            except: st.error("Login Failed. Check Internet or Password.")
    st.stop()
...
if st.session_state.user_email == "amit@admin.com":
    ...
    with col_l:
        ...
        if c2.button("⏹️ STOP ENGINE", use_container_width=True):
            st.session_state.running = False
            st.rerun() # Refresh to stop background loop immediately
...
# --- 7. HYBRID DUAL LOOP (Engine Logic) ---
